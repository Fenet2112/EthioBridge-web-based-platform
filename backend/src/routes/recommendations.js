...
async function fallbackProducts(limit = 10) {
  const result = await pool.query(`
    ...
    GROUP BY p.id, p.name, p.category, p.price, p.unit, p.image_url,
...
async function fallbackIndustries(limit = 10) {
  const result = await pool.query(`
...
router.get("/recommendations/products", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    try {
      const recs = await fallbackProducts(parseInt(top_n) || 10);
...
router.get("/recommendations/industries", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    console.warn("ML service unavailable, using fallback:", err.message);
    try {
      const recs = await fallbackIndustries(parseInt(top_n) || 10);
      return res.json({ source: "fallback", recommendations: recs });
    } catch (dbErr) {
      return res.status(500).json({ message: "Recommendation service error" });
    }

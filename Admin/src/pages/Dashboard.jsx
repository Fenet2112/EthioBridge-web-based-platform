...
function Dashboard() {
  ...
  const handleUserReject = async () => {
    ...
    try {
      const res = await fetch(`${API}/api/admin/users/${rejectModal}/reject`, {
        method: "PATCH", headers: { "Content-Type": "application/json", Authorization: `Bearer ${tok()}` },
  ...
  return (
    <div className="admin-dashboard">
      ...
      <aside className="admin-sidebar">
        <div className="admin-logo"><span>🌉</span><span>EthioBridge Admin</span></div>
        <nav className="admin-nav">
          <button className={view === "users"     ? "active" : ""} onClick={() => { setView("users");     setFilter("pending"); }}>👥 Approvals</button>
          <button className={view === "purchases" ? "active" : ""} onClick={() => { setView("purchases"); setPrFilter("pending"); }}>📋 Purchase Requests</button>
          <button className={view === "manage"    ? "active" : ""} onClick={() => { setView("manage"); }}>🛡️ User Management</button>
        </nav>
      ...
      <main className="admin-main">
                  ...
                  <div className="user-card" key={user.id}>
                    ...
                    {user.status === "pending" && (
                      <div className="user-actions">
                        <button className="approve-btn" onClick={() => handleUserApprove(user.id)} disabled={actionLoading === user.id + "-approve"}>
        ...
        {view === "purchases" && (
          <>
            <div className="admin-topbar">
              <div><h1>Purchase Requests</h1><p>{purchaseRequests.length} total requests</p></div>
              <div className="topbar-actions">
                ...
                {purchaseRequests.map(req => (
                  <div key={req.id} className="purchase-card">
                    ...
                    <div className="purchase-cards-grid">
                      <div className="info-card industry-card"><div style={{ fontSize: "24px", marginBottom: "8px" }}>🏭</div><div className="info-card-label">INDUSTRY</div><div className="info-card-value">{req.industry_name}</div><div className="info-card-sub">{req.sector}</div></div>
                      <div className="info-card stakeholder-card"><div style={{ fontSize: "24px", marginBottom: "8px" }}>🤝</div><div className="info-card-label">STAKEHOLDER</div><div className="info-card-value">{req.organization_name}</div><div className="info-card-sub">{req.full_name}</div></div>
                    </div>
          ...
          <>
            <div className="admin-topbar">
              <div>
                ...
                <p>{filteredUsers.length} of {allUsers.length} users shown</p>
              </div>
                    ...
                    {filteredUsers.map(u => (
                      <tr key={u.id} className={`um-row um-row-${u.status}`}>
                        ...
                        <td className="um-name">
                          <span className="um-role-icon">{u.role === "industry" ? "🏭" : "🤝"}</span>
                          {u.display_name || "—"}
                        </td>
                        ...
                        <td>
                          {u.identity_verified || u.is_verified
                            ? <span className="um-verified">✓ Verified</span>
                            : <span className="um-unverified">— Not verified</span>}
                        </td>
                        <td className="um-date">{new Date(u.created_at).toLocaleDateString()}</td>
                        <td>
                          <div className="um-actions">
                            {u.status !== "banned" && (
                              <button className="um-btn um-ban" onClick={() => { setActionModal({ user: u, action: "ban" }); setActionReason(""); }} title="Ban user">🚫 Ban</button>
                            )}
                            {u.status !== "suspended" && u.status !== "banned" && (
                              <button className="um-btn um-suspend" onClick={() => { setActionModal({ user: u, action: "suspend" }); setActionReason(""); }} title="Suspend user">⏸ Suspend</button>
                            )}
                            {(u.status === "suspended" || u.status === "banned") && (
                              <button className="um-btn um-activate" onClick={() => { setActionModal({ user: u, action: "activate" }); setActionReason(""); }} title="Restore access">🔓 Activate</button>
                            )}
                          </div>
                        </td>
                      </tr>
        ...
        <div className="modal-overlay" onClick={() => setRejectModal(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-actions">
              ...
              <button className="modal-reject" onClick={rejectModal.type === "user" ? handleUserReject : handleReject} disabled={actionLoading === rejectModal.id + "-reject"}>
      ...
      {/* ── Ban / Suspend / Activate Confirm Modal ── */}
      {actionModal && (
        <div className="modal-overlay" onClick={() => setActionModal(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <p className="um-confirm-name">
              {actionModal.user.display_name || actionModal.user.email}
            </p>
            <p className="um-confirm-desc">
              {actionModal.action === "ban"
                ? "This user will be permanently blocked from logging in or using the platform."
                : actionModal.action === "suspend"
                ? "This user will be temporarily restricted. You can reactivate them at any time."
            ...
            {actionModal.action !== "activate" && (
              <div className="um-reason-field">
                ...
                <textarea
                  ...
                  placeholder={`Reason for ${actionModal.action}ning this user...`}
                  rows={3}
                />
              </div>
            )}
            <div className="modal-actions">
              ...
              <button
                className={actionModal.action === "activate" ? "approve-btn" : "reject-btn"}
        ...
        <div className="modal-overlay" onClick={() => setViewDetailsModal(null)}>
          <div className="modal details-modal" onClick={e => e.stopPropagation()}>
            <div className={`details-modal-header ${viewDetailsModal.role}`}>
              ...
              <div style={{ fontSize: "48px", marginBottom: "10px" }}>{viewDetailsModal.role === "industry" ? "🏭" : "🤝"}</div>
            ...
            <div style={{ padding: "30px" }}>
              <div style={{ marginBottom: "20px" }}><StatusBadge status={viewDetailsModal.status} /></div>
              <div style={{ display: "grid", gap: "20px" }}>
                ...
                {viewDetailsModal.role === "stakeholder" && viewDetailsModal.id_document_url && (
                  <div className="details-section">
                    <h3>🛡️ Identity Document</h3>
                    <p style={{ fontSize: "13px", color: "#6c757d", marginBottom: "12px" }}>
                      Type: <strong>{viewDetailsModal.id_document_type?.replace(/_/g, " ") || "N/A"}</strong>
                      {viewDetailsModal.identity_verified && <span style={{ marginLeft: "12px", background: "#e8f5e9", color: "#0a5c2f", padding: "2px 10px", borderRadius: "50px", fontSize: "12px", fontWeight: 700 }}>✓ Verified</span>}
                    </p>
                    {viewDetailsModal.id_document_url.match(/\.(jpg|jpeg|png)$/i)
                      ? <a href={`${API}${viewDetailsModal.id_document_url}`} target="_blank" rel="noopener noreferrer"><img src={`${API}${viewDetailsModal.id_document_url}`} alt="ID Document" className="id-doc-preview" /></a>
                      : <a href={`${API}${viewDetailsModal.id_document_url}`} target="_blank" rel="noopener noreferrer" className="id-doc-link">📄 View Document (PDF)</a>}
                  </div>
                )}
              </div>
            </div>
            <div className="details-modal-footer">
              ...
              {viewDetailsModal.status === "pending" && (
                <>
                  <button className="approve-btn" style={{ flex: "unset", padding: "10px 20px" }} onClick={() => { handleUserApprove(viewDetailsModal.id); setViewDetailsModal(null); }}>✓ Approve</button>
                  <button className="reject-btn" style={{ flex: "unset", padding: "10px 20px" }} onClick={() => { setRejectModal({ id: viewDetailsModal.id, type: "user" }); setViewDetailsModal(null); setRejectReason(""); }}>✕ Reject</button>
                </>
              )}
            </div>
          </div>
